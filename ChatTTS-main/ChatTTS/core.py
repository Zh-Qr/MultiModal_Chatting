...
class Chat:
    ...
    def check_model(self, level=logging.INFO, use_decoder=False):
        ...
        if not not_finish:
            self.logger.log(level, 'All initialized.')
    ...
    def load_models(self, source='huggingface', force_redownload=False, local_path='<LOCAL_PATH>', **kwargs):
        if source == 'huggingface':
            ...
            if download_path is None or force_redownload:
                self.logger.info('Download from HF: https://huggingface.co/2Noise/ChatTTS')
                download_path = snapshot_download(repo_id="2Noise/ChatTTS", allow_patterns=["*.pt", "*.yaml"])
            else:
                self.logger.info(f'Load from cache: {download_path}')
        elif source == 'local':
            self.logger.info(f'Load from local: {local_path}')
    ...
    def infer(
        ...
        for i, t in enumerate(text):
            ...
            if len(invalid_characters):
                self.logger.warning(f'Invalid characters found! : {invalid_characters}')
        ...
        if lang not in self.normalizer:
            if lang == 'zh':
                try:
                    ...
                    self.normalizer[lang] = lambda x: x  # 添加一个默认的正规化方法
            else:
                try:
                    from nemo_text_processing.text_normalization.normalize import Normalizer
                    self.normalizer[lang] = partial(
                        Normalizer(input_case='cased', lang=lang).normalize, verbose=False, punct_post_process=True
                    )
                except ImportError:
                    self.logger.warning(
